...
def generar_erp():
    ...
    # Defaults desde variables de entorno, sobreescribibles por el form
    default_pub = float(os.environ.get("MULT_PUBLICO", 2.3))
    default_may = float(os.environ.get("MULT_MAYOREO", 1.35))
    try: mp=float(request.form.get("mult_publico", default_pub)); mm=float(request.form.get("mult_mayoreo", default_may))
    except: return jsonify({"error":"Multiplicadores invalidos"}),400
    try:
        pdf=f.read()
        # Auto-detectar formato si no se especificó o como fallback
        if empresa == "alessia":
            data = parse_alessia(pdf)
        else:
            data = parse_cfdi(pdf)

        # Fallback: si no encontró partidas, intentar el otro parser
        if not data["items"]:
            data = parse_alessia(pdf) if empresa == "vifer" else parse_cfdi(pdf)

        if not data["items"]:
            return jsonify({"error": "No se encontraron partidas. Verifica que el PDF sea una factura CFDI o un pedido SAP de Alessia."}), 422
